...
Após atingir certos critérios, você receberá convites de [faction](../basic/factions.md) do jogo.
Juntar-se a [faction](../basic/factions.md) e trabalhar para elas desbloqueará vários [augmentations](../basic/augmentations.md),
que são comprados e "instalados", adicionando um bônus persistente a [stats](../basic/stats.md) e outras habilidades. Trabalhar com Facções e instalar Aumentos é um passo básico para progredir no Bitburner.
...
Para resumir brevemente: Cada [Server](../basic/servers.md) tem um nível de segurança que afeta o quão difícil é hackear.
Cada [Server](../basic/servers.md) também tem uma certa quantia de dinheiro, bem como uma quantia máxima de dinheiro que pode conter.
[Hacking](../basic/hacking.md) um [Server](../basic/servers.md) rouba uma porcentagem do dinheiro daquele [Server](../basic/servers.md).
A função `hack()` é usada para hackear um [Server](../basic/servers.md).
A função `grow()` é usada para aumentar a quantia de dinheiro disponível em um [Server](../basic/servers.md).
A função `weaken()` é usada para diminuir o nível de segurança de um [Server](../basic/servers.md).
...
Este primeiro comando define uma string que contém nosso [Server](../basic/servers.md) alvo.
Esse é o [Server](../basic/servers.md) que vamos [Hacking](../basic/hacking.md).
Por enquanto, está definido como `"n00dles"` porque esse é o único [Server](../basic/servers.md) com um nível de hacking necessário de `1`.
Se você quiser [Hacking](../basic/hacking.md) um [Server](../basic/servers.md) diferente, simplesmente altere esta variável para ser o nome do host de outro [Server](../basic/servers.md).

    const moneyThresh = ns.getServerMaxMoney(target);

Este segundo comando define um valor numérico que representa a quantia mínima de dinheiro que deve estar disponível no [Server](../basic/servers.md) de destino para que nosso [Script](../basic/scripts.md) o [Hacking](../basic/hacking.md).
Se o dinheiro disponível no [Server](../basic/servers.md) de destino for menor que este valor, então nosso [Script](../basic/scripts.md) fará `grow()` no [Server](../basic/servers.md) em vez de [Hacking](../basic/hacking.md).
Ele é definido como a quantia máxima de dinheiro que pode estar disponível no [Server](../basic/servers.md).
A função `getServerMaxMoney()` é usada para encontrar este valor:

    const securityThresh = ns.getServerMinSecurityLevel(target);

Este terceiro comando define um valor numérico que representa o nível mínimo de segurança que o [Server](../basic/servers.md) de destino pode ter.
Se o nível de segurança do [Server](../basic/servers.md) de destino for maior que este valor, então nosso [Script](../basic/scripts.md) irá `weaken()` o servidor antes de fazer qualquer outra coisa.
...
Esta seção de código é usada para obter acesso root no [Server](../basic/servers.md) de destino.
Isso é necessário para [Hacking](../basic/hacking.md).
...
Isso mostrará informações detalhadas sobre alguns [Servers](../basic/servers.md) na rede.
...
Aumentar suas stats aumenta sua chance de cometer um [crime](../basic/crimes.md) com sucesso.
...
Se você não quer cometer [crimes](../basic/crimes.md), há outra opção - trabalhar para uma [company](../basic/companies.md).
Isso não será nem de longe tão lucrativo quanto [crimes](../basic/crimes.md), mas dará à [company](../basic/companies.md) [reputation](../basic/reputation.md).
...
Lembre-se de que se você tiver o programa `AutoLink`, você pode simplesmente clicar no nome do host de um [Server](../basic/servers.md) após executar `scan-analyze` para se conectar a ele.
...
(Você pode ver essas stats na aba `Active Scripts`).
...
- Nesta fase do jogo, suas stats de combate (força, defesa, etc.) não são tão úteis quanto suas stats de hacking.
Não invista muito tempo ou dinheiro para ganhar exp de stats de combate.
